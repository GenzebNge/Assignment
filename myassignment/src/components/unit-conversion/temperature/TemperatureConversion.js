...
export class TemperaturConvertion extends Component {
  constructor(props) {
    super(props);
  ...
  convertHandler = () => {
    ...
    if (
      this.state.convertedFrom === "kelvine" &&
      this.state.convertedTo === "Fahrenheit"
    ) {
      tempResult = initialVal * (9 / 5) - 459.67;
    }
    if (
      this.state.convertedFrom === "Fahrenheit" &&
      this.state.convertedTo === "kelvine"
    ) {
      tempResult = (initialVal + 460) * (5 / 9);
    }
    if (
      this.state.convertedFrom === "kelvine" &&
      this.state.convertedTo === "Celsius"
    ) {
      tempResult = initialVal - 273.15;
    }
    if (
      this.state.convertedFrom === "Celsius" &&
      this.state.convertedTo === "kelvine"
    ) {
      tempResult = initialVal + 273.15;
    }
    if (
      this.state.convertedFrom === "kelvine" &&
      this.state.convertedTo === "Rankine"
    ) {
      tempResult = (initialVal * 9) / 5;
    }
    if (
      this.state.convertedFrom === "Rankine" &&
      this.state.convertedTo === "kelvine"
    ) {
  ...
  render() {
    return (
      <div className="container">
        <div className="row">
          <Temperature
            scales={this.state.tempScaleOptions}
          ...
          />
           <span> {this.state.Result} {this.state.convertedResult}</span>
...
export default TemperaturConvertion;
